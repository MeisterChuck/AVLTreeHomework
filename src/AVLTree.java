public class AVLTree<E extends Comparable<? super E>>{
    ...
    * 
    * This method rotates the subtree balancing it to within a margin
    * of |1|.
    */
   public void rotate(AVLNode<E> rotateBase, AVLNode<E> rootAbove){
       ...
       AVLNode<E> child = (balance < 0) ? rotateBase.getLeft() : 
               rotateBase.getRight();
       ...
       // Both the child and grandchild are on the left side, so rotate the
       // child up to the root position
       if(balance < -1 && childBalance < 0){
           ...
           return;
       // The child on the left side, but the grandchild is on the right
       // side, so rotate the grandchold up to the child position so the
       // condition of the first if statement is satisfied, then recurse to
       // have the first if statement evaluated
       }else if(balance < -1 && childBalance > 0){
           grandChild = child.getRight();
           rotateBase.setLeftNode(grandChild);
       ...
       // The child is of the right side, but the grandchild is on the left
       // side, so rotate the grandchild up to the child position so the
       // condition of the second if statement is satisfied, then recurse to
       // have the second if statement evaluated
       }else if(balance > 1 && childBalance < 0){
           grandChild = child.getLeft();
           rotateBase.setRightNode(grandChild);
    ...
    * 
    * This method recursively traverses the Tree, inserting the element at the
    * appropriate spot and incrementing the balance factors for the subtrees as
    * it evaluates. The Tree will then recursively rebalance as necessary.
    */
   private void insert(E element, AVLNode<E> temp){
        ...
        // Travel left or right based on the comparison of element to
        // temp.element
        // Remember that left means that element <= temp.element and right
        // means element > temp.element
        int compare = element.compareTo(temp.getElement());
        
        // Travel to the left of the Tree, inserting of the bottom has been
        // reached
        if(compare <= 0){
        ...
        // Otherwise, travelling to the right of the Tree, inserting if the
    ...
    * @param temp: The root of the subtree
    * 
    * This method recursively traverses the AVLTree based on the ordering of
    * the element with respect to the Tree's elements. If the element is not
    * found, then nothing happens.
    * 
    * Otherwise, the element is removed, and either the far-right element on
    * its left child or the far left element on its right child replaces it.
    */
   private void remove(E element, AVLNode<E> temp){
       ...
       // If the root is perfectly balanced, slide the left Node up and
       // reinsert the left.right element if necessary
       ...
       // If the element is found and the root is not perfectly balanced, 
       // remove it using enactRemoval()
    ...
    * @param direction: Fbalse if remove is to the left of parent, true otherwise
    * 
    ...
    */
    private void enactRemoval(AVLNode<E> parent, AVLNode<E> remove, boolean direction){
        ...
        // If the Node to removremove(7);e is not a leaf, find the appropriate successor
        ...
        // Otherwise, update its left node with successor
        }else{
            parent.setLeftNode(temp);
        }
        
        // Update temp to point to remove's children
        if(temp != null){
            if(temp != left){
        ...
        // Finally, discard those references from remove so that the removed
        // Node is garbage collected sooner
    ...
    private AVLNode<E> findSuccessor(AVLNode<E> root){
        ...
        // Find the farthest left-Node on the right side, or the farthest
        // right-Node on the left side
        while((temp.getRight() != null && !direction) || (temp.getLeft() != null && direction)){
            parent = temp;
            temp = (direction) ? temp.getLeft() : temp.getRight();
        }
        
        // Finally, update the successor's parent's references to adjust for a
        // left child on the right node, or a right child on the left node
     ...
     * 
     * The contains method simply tracerses the binary search tree based on
     * element's relation to the AVLNodes in the Tree until a match is found or
     * it hits the bottom of the Tree.
     */
